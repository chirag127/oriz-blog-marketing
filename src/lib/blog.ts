...
/** Map a raw category string → one of the five marketing verticals. */
export function verticalFor(category: string): Vertical {
  const c = category.toLowerCase()
  if (/(email|newsletter|list|deliverab|inbox|subscriber)/.test(c)) return 'health'
  if (/(analytic|metric|data|measure|ga4|attribution|conversion|report)/.test(c)) return 'finance'
  if (/(seo|search|rank|serp|keyword|technical|link|backlink)/.test(c)) return 'dev'
  if (/(content|copy|writing|blog|social|brand|video|creative)/.test(c)) return 'culture'
  return 'personal'
}
